...
func NewOptionalBoolField(val func(r {{.Type}}) *bool, col string) *OptionalBoolField {
...
func (f *OptionalBoolField) Add(r {{.Type}}) {
	v := f.val(r)
	if v != nil {
...
func (f *OptionalBoolField) Write(w io.Writer, meta *parquet.Metadata, pos int) error {
	buf := bytes.Buffer{}
	wc := &WriteCounter{w: &buf}
...
func NewStringField(val func(r {{.Type}}) string, col string) *RequiredStringField {
...
func (f *RequiredStringField) Add(r {{.Type}}) {
	f.vals = append(f.vals, f.val(r))
}

func (f *RequiredStringField) Write(w io.Writer, meta *parquet.Metadata, pos int) error {
	buf := bytes.Buffer{}
	wc := &WriteCounter{w: &buf}
