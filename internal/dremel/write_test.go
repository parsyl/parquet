...
func TestWrite(t *testing.T) {
	...
	x.ID = vals[0]
		...
		},
		{
			...
			result: `func writeID(x *Person, vals []int32, def uint8) bool {
	switch def {
	case 1:
		v := vals[0]
		x.ID = &v
	...
	x.Other.Hobby.Difficulty = vals[0]
		...
		},
		{
			...
			result: `func writeHobbyDifficulty(x *Person, vals []int32, def uint8) bool {
	switch def {
	case 1:
		if x.Hobby == nil {
			x.Hobby = &Hobby{}
	...
	return false
		...
		},
		{
			...
			result: `func writeHobbyName(x *Person, vals []string, def uint8) bool {
	switch def {
	case 1:
		v := vals[0]
		if x.Hobby == nil {
	...
	return false
		...
		},
		{
			...
			result: `func writeFriendHobbyName(x *Person, vals []string, def uint8) bool {
	switch def {
	case 1:
		if x.Friend == nil {
			x.Friend = &Entity{}
	...
	return false
		...
		},
		{
			...
			result: `func writeFriendHobbyNameFirst(x *Person, vals []string, def uint8) bool {
	switch def {
	case 1:
		if x.Friend == nil {
			x.Friend = &Entity{}
